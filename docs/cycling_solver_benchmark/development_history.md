# Historique du benchmark des solveurs du RHO de pédalage FES

> [!IMPORTANT]
> Ce document est une archive chronologique des développements, campagnes CI,
> erreurs découvertes et options réfutées. Il ne définit plus la méthode à
> utiliser. La configuration recommandée et ses critères scientifiques sont
> décrits dans le [README du benchmark](README.md). Une ancienne « référence »
> reste documentée ici pour assurer la traçabilité, mais elle ne doit pas être
> reproduite lorsqu'une erreur de modèle ou de discrétisation a été démontrée.
> Le [point de reprise](resume_and_todo.md) contient le bilan condensé et les
> prochaines tâches; il doit être lu avant de reprendre une campagne.
